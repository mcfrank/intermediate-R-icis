...
We want you to ask questions in the chat. Other students will learn from your questions because they have the same questions too probably! If you are feeling shy (or have issues with software installs or bugs that are specific to your computer or a particular line of code that isn't working) then you can private chat to Mika Braginsky, Georgia Loukatou, or George Kachergis, our TAs!

## Misc

The best reference for this material is Hadley Wickham's [R for data scientists](http://r4ds.had.co.nz/). My contribution here is to translate this reference for psychology.s
...
to_inches <- function (cm) { cm / 2.54 }

# first compute mean height in inches the normal, nested way  
round(mean(to_inches(starwars$height), na.rm=TRUE), digits = 2)
...
ggplot(starwars, aes(x = mass, y = height, label = name)) + 
  geom_point() + 
  geom_text()
```
